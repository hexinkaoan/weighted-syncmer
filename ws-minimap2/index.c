...
mm_idx_t *mm_idx_init(int w, int k, int b, int flag, int s, int t)
{
	mm_idx_t *mi;
	if (k*2 < b) b = k * 2;
	if (w < 1) w = 1;
	mi = (mm_idx_t*)calloc(1, sizeof(mm_idx_t));
	mi->w = w, mi->k = k, mi->b = b, mi->flag = flag, mi->s = s, mi-> t = t;
...
static void *worker_pipeline(void *shared, int step, void *in)
{
    ...
    if (step == 0) { // step 0: read sequences
		...
		for (i = 0; i < s->n_seq; ++i) {
			...
			if (t->l_seq > 0)
				mm_sketch(0, t->seq, t->l_seq, p->mi->k, p->mi->s, p->mi->t, t->rid, p->mi->flag&MM_I_HPC, &s->a, p->mi);
...
mm_idx_t *mm_idx_gen(mm_bseq_file_t *fp, int w, int k, int b, int flag, int mini_batch_size, int n_threads, uint64_t batch_size, int s, int t, const char *kmer_freq_filename)
{
	pipeline_t pl;
	...
	pl.fp = fp;
	pl.mi = mm_idx_init(w, k, b, flag, s, t);
...
mm_idx_t *mm_idx_build(const char *fn, int w, int k, int flag, int n_threads, int s, int t) // a simpler interface; deprecated
{
	mm_bseq_file_t *fp;
	mm_idx_t *mi;
	fp = mm_bseq_open(fn);
	if (fp == 0) return 0;
	mi = mm_idx_gen(fp, w, k, 14, flag, 1<<18, n_threads, UINT64_MAX, s, t,NULL);
	mm_bseq_close(fp);
	return mi;
}

mm_idx_t *mm_idx_str(int w, int k, int is_hpc, int bucket_bits, int n, const char **seq, const char **name, int s, int t)
{
	uint64_t sum_len = 0;
	mm128_v a = {0,0,0};
	mm_idx_t *mi;
	...
	mi = mm_idx_init(w, k, bucket_bits, flag, s, t);
	mi->n_seq = n;
	...
	for (i = 0, sum_len = 0; i < n; ++i) {
		...
		if (p->len > 0) {
			a.n = 0;
			mm_sketch(0, s, p->len, mi->k, mi->s, mi->t, i, is_hpc, &a,mi);
...
mm_idx_t *mm_idx_load(FILE *fp)
{
	...
	mi = mm_idx_init(x[0], x[1], x[2], x[4], 0, 0);
	mi->n_seq = x[3];
...
mm_idx_t *mm_idx_reader_read(mm_idx_reader_t *r, int n_threads, const char *kmer_freq_filename)
{
	...
	if (r->is_idx) {
		...
		mi = mm_idx_gen(r->fp.seq, r->opt.w, r->opt.k, r->opt.bucket_bits, r->opt.flag, r->opt.mini_batch_size, n_threads, r->opt.batch_size, r->opt.s, r->opt.t, kmer_freq_filename);
	if (mi) {
