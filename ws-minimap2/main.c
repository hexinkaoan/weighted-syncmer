...
{
	const char *opt_str = "2aSDw:W:k:K:t:r:f:Vv:g:G:I:d:XT:s:x:Hcp:M:n:z:A:B:b:O:E:m:N:Qu:R:hF:LC:yYPo:e:U:J:";
	...
	double index_start_time, index_end_time, mapping_start_time, mapping_end_time;
	...
	while ((c = ketopt(&o, argc, argv, 1, opt_str, long_options)) >= 0) {
		if (c == 'w') ipt.w = atoi(o.arg);
            ...
            ipt.t = atoi(o.arg); 
            if (ipt.t > 0 && ipt.s > 0) 
			fprintf(stderr, "SYNCT is %d , SYNCS is %d \n", ipt.t ,ipt.s);
        }
        else if (c == 402) {
            ipt.s = atoi(o.arg); 
        }
...
//		fprintf(fp_help, "    -v INT       verbose level [%d]\n", mm_verbose);
	...
	while ((mi = mm_idx_reader_read(idx_rdr, n_threads, opt.kmer_freq_filename)) != 0) {
		...
		if (!(opt.flag & MM_F_FRAG_MODE)) {
			mapping_start_time = realtime();
			for (i = o.ind + 1; i < argc; ++i) {
				ret = mm_map_file(mi, argv[i], &opt, n_threads);
				if (ret < 0) break;
			}
			mapping_end_time = realtime();
		} else {
			mapping_start_time = realtime();
	...
	if (mm_verbose >= 3) {
		...
		// 输出索引构建时间和比对时间
		fprintf(stderr, "[M::%s] Indexing time: %.3f sec\n", __func__, index_end_time - index_start_time);
		fprintf(stderr, "[M::%s] Mapping time: %.3f sec\n", __func__, mapping_end_time - mapping_start_time);
	}
	return 0;
}
