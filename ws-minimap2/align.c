...
static void mm_align1(void *km, const mm_mapopt_t *opt, const mm_idx_t *mi, int qlen, uint8_t *qseq0[2], mm_reg1_t *r, mm_reg1_t *r2, int n_a, mm128_t *a, ksw_extz_t *ez, int splice_flag)
{
	...
	if (is_sr) {//短读
		...
		// compute rs0 and qs0
		rs0 = (int32_t)a[r->as].x + 1 - (int32_t)(a[r->as].y>>32&0xff);//rs0是参考序列的起始位置
		qs0 = (int32_t)a[r->as].y + 1 - (int32_t)(a[r->as].y>>32&0xff);//qs0是查询序列的起始位置
		if (rs0 < 0) rs0 = 0; // this may happen when HPC is in use
		assert(qs0 >= 0); // this should never happen, or it is logic error
		rs1 = qs1 = 0;
		for (i = r->as - 1, l = 0; i >= 0 && a[i].x>>32 == a[r->as].x>>32; --i) { // inspect nearby seeds找到一个新的起始位置rs1和qs1。
		...
		if (qs > 0 && rs > 0) {//确定是否需要进行左扩展：确保扩展后的区域不会超过最大间隔（opt->max_gap）
		...
		// compute re0 and qe0
		re0 = (int32_t)a[r->as + r->cnt - 1].x + 1;//计算参考序列的结束位置re0
		qe0 = (int32_t)a[r->as + r->cnt - 1].y + 1;//计算查询序列的结束位置qe0
		...
		if (qe < qlen && re < (int32_t)mi->seq[rid].len) {//确定是否需要进行右扩展：确保在查询序列和参考序列的范围内扩展
	...
	if (a[r->as].y & MM_SEED_SELF) {//检查是否自比对:（MM_SEED_SELF），限制最大扩展长度，防止过度扩展导致比对错误。
	...
	assert(re0 > rs0);//确保re0大于rs0，然后为比对区域分配内存
	tseq = (uint8_t*)kmalloc(km, re0 - rs0);
	junc = (uint8_t*)kmalloc(km, re0 - rs0);

	if (qs > 0 && rs > 0) { // left extension; probably the condition can be changed to "qs > qs0 && rs > rs0".包括获取查询序列和参考序列的子序列，反向序列，以及调用比对函数mm_align_pair。
		if (opt->flag & MM_F_QSTRAND) {
			qseq = &qseq0[0][qs0];
...
//..........----------
	...
	if (!dropped && qe < qe0 && re < re0) { // right extension.包括获取查询序列和参考序列的子序列，以及调用比对函数mm_align_pair。
		if (opt->flag & MM_F_QSTRAND) {
			qseq = &qseq0[0][qe];
...
//更新比对结果并进行后处理
	...
	if (r->p) {
		...
		mm_update_extra(r, qseq, tseq, mat, opt->q, opt->e, opt->flag & MM_F_EQX, !(opt->flag & MM_F_SR)); //更新比对结果中的额外信息，包括打分矩阵、比对得分和比对模式
